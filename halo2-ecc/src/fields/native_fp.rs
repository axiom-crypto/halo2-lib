use super::{FieldChip, PrimeField};
...
#[derive(Clone, Debug)]
pub struct NativeFieldChip<'range, F: PrimeField> {
    pub range: &'range RangeChip<F>,
    pub native_modulus: BigUint,
    _marker: PhantomData<F>,
}

impl<'range, F: PrimeField> NativeFieldChip<'range, F> {
...
impl<'range, F: PrimeField> FieldChip<F> for NativeFieldChip<'range, F> {
