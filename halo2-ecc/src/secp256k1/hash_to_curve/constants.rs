...
pub(crate) fn get_Z<F: BigPrimeField>(
    ...
    limbs_le_to_bn(ctx, fp_chip, z.as_slice())
}
...
pub(crate) fn get_B<F: BigPrimeField>(
    ...
    let b = [1771, 0, 0, 0].iter().map(|v| ctx.load_constant(F::from(*v))).collect::<Vec<_>>();

    limbs_le_to_bn(ctx, fp_chip, b.as_slice())
}
...
pub(crate) fn get_C1<F: BigPrimeField>(
    ...
    limbs_le_to_bn(ctx, fp_chip, c1.as_slice())
}
...
pub(crate) fn get_C2<F: BigPrimeField>(
    ...
    limbs_le_to_bn(ctx, fp_chip, c2.as_slice())
}
...
pub(crate) fn get_k_1_0<F: BigPrimeField>(
    ...
    limbs_le_to_bn(ctx, fp_chip, k.as_slice())
}
