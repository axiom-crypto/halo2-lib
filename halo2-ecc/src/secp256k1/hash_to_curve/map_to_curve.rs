...
fn xy2_selector<F: BigPrimeField>(
    ...
    let gx1_sqrt = sqrt(ctx, fp_chip, gx1.clone());
    let gx2_sqrt = sqrt(ctx, fp_chip, gx2.clone());

    let sq_gx1_sqrt = fp_chip.mul(ctx, gx1_sqrt.clone(), gx1_sqrt);
    let sq_gx2_sqrt = fp_chip.mul(ctx, gx2_sqrt.clone(), gx2_sqrt);

    let s1 = fp_chip.is_equal(ctx, sq_gx1_sqrt.clone(), gx1.clone());
    let s2 = fp_chip.is_equal(ctx, sq_gx2_sqrt.clone(), gx2.clone());
...
pub(crate) fn map_to_curve<F: BigPrimeField>(
    ...
    // Step 19: y = sqrt(y2)
    let y = sqrt(ctx, fp_chip, y2.clone());
    let y_sq = fp_chip.mul(ctx, y.clone(), y.clone());
    ...
    let sgn_y = fp_chip.is_even(ctx, y.clone());
    ...
    let (x_mapped, y_mapped) = iso_map(ctx, fp_chip, x.clone(), y.clone());
