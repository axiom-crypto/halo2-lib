...
The default features are: "jemallocator" and "display".
You can turn off "display" for a very small performance increase, where certain statistics about the circuit are not
computed and printed.

This crate uses our [`halo2_proofs`](https://github.com/axiom-crypto/halo2) fork.
...
```
cargo test --release --no-default-features --features "jemallocator" -- --nocapture bench_secp256k1_ecdsa
```
...
```
cargo test --release --no-default-features --features "mimalloc" -- --nocapture bench_secp256k1_ecdsa
```
...
```
cargo test --release --no-default-features --features "jemallocator" -- --nocapture bench_pairing
```
...
```
cargo test --release --no-default-features --features "mimalloc" -- --nocapture bench_pairing
```
...
```
cargo test --release --no-default-features --features "mimalloc" -- --nocapture bench_msm
```
