...
pub fn test_check_less_than<F: ScalarField>(k: usize, lookup_bits: usize, a: QuantumCell<F>, b: QuantumCell<F>, num_bits: usize) {
    std::env::set_var("LOOKUP_BITS", lookup_bits.to_string());
    let mut builder = GateThreadBuilder::mock();
    let ctx = builder.main(0);
    let chip = RangeChip::default(lookup_bits);
...
pub fn test_check_big_less_than_safe<F: ScalarField + BigPrimeField>(k: usize, lookup_bits: usize, a_val: F, b: u64) {
    std::env::set_var("LOOKUP_BITS", lookup_bits.to_string());
    let mut builder = GateThreadBuilder::mock();
    let ctx = builder.main(0);
    let chip = RangeChip::default(lookup_bits);
...
pub fn test_is_less_than<F: ScalarField>(inputs: ([QuantumCell<F>; 2], usize, usize)) -> F {
    std::env::set_var("LOOKUP_BITS", inputs.2.to_string());
    let mut builder = GateThreadBuilder::mock();
    let ctx = builder.main(0);
