...
use crate::{gates::{GateChip, GateInstructions, builder::{GateCircuitBuilder, GateThreadBuilder}, tests::{Fr, flex_gate_test, range_gate_test}}, utils::ScalarField};
...
prop_compose! {
    ...
    fn idx_to_indicator_strat(max_size: usize)
        (k in 8u32..=20u32, idx_val in prop::sample::select(vec![Fr::zero(), Fr::one(), Fr::random(OsRng)]), len in 2usize..=max_size)
        (k in Just(k), idx in 0..len, idx_val in Just(idx_val), len in Just(len), mut witness_vals in arb_indicator::<Fr>(len)) -> (u32, usize, usize, Vec<Fr>) {
...
fn check_instance(idx: usize, len: usize, ind_witnesses: &[Fr]) -> bool {   
    // check that:
...
fn prop_test_idx_to_indicator_gen(k: u32, len: usize, idx: usize, ind_witnesses: &[Fr]) -> bool {
    println!("ind_witnesses: {:?}", ind_witnesses);
    ...
    let is_valid_witness = check_instance(idx, len, ind_witnesses);
    match MockProver::run(k as u32, &circuit, vec![]).unwrap().verify() {
            ...
            Ok(_) => {
                println!("Ok: is_valid_witness {:?}", is_valid_witness);
                is_valid_witness == true
            },
            // if the proof is invalid, ignore
            Err(e) =>  {
                println!("Err: is_valid_witness {:?}", is_valid_witness);
                println!("Error: {:?}", e);
...
proptest! {
    ...
    fn prop_test_range_check(inputs in (rand_fr(), any::<usize>().prop_filter("Non-zero upper bound", |x| *x != 0usize))) {
        prop_assert_eq!(range_gate_test::test_range_check((inputs.0, inputs.1)), ());
    }
    ...
    fn test_neg_idx_to_indicator_gen((k, len, idx, witness_vals) in idx_to_indicator_strat(100)) {
        prop_assert!(prop_test_idx_to_indicator_gen(k, len, idx, witness_vals.as_slice()));
    }
}